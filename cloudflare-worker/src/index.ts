/**
 * Ollama Android Client GitHub feedback relay.
 *
 * The Android app's "Report a Problem" feature used to embed a GitHub personal access
 * token directly in the APK (BuildConfig.GITHUB_API_TOKEN, sent client-side as an
 * Authorization header to api.github.com) so it could create/read issues and comments,
 * and push screenshots via the Contents API. That token has issue-write and
 * repo-content-write scope — extractable from any downloaded APK in seconds (`strings`
 * on the dex finds any literal compiled into the app), handing every installer of the
 * app write access to this repository. It has been shipping in every CI release build.
 *
...
const GITHUB_REPO = 'ollama-android-client';
...
function githubHeaders(env: Env): HeadersInit {
  return {
    ...
    'User-Agent': 'ollama-android-client-cloudflare-worker',
